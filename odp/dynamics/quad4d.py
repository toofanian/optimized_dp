...
class Quad4D(OdpDynamics):
    ...
    def opt_ctrl(self, t, state, spat_deriv):
        ...
        SumUU[0] = spat_deriv[1]*(((hcl.cos(state[2]) / self.mass * self.max_thrust) + (hcl.cos(state[2]) / self.mass * self.max_thrust))) + spat_deriv[3]*(((-self.length_between_copters / self.moment_of_inertia * self.max_thrust) + (self.length_between_copters / self.moment_of_inertia * self.max_thrust)))
        SumUL[0] = spat_deriv[1]*(((hcl.cos(state[2]) / self.mass * self.max_thrust) + (hcl.cos(state[2]) / self.mass * self.min_thrust))) + spat_deriv[3]*(((-self.length_between_copters / self.moment_of_inertia * self.max_thrust) + (self.length_between_copters / self.moment_of_inertia * self.min_thrust)))
        SumLU[0] = spat_deriv[1]*(((hcl.cos(state[2]) / self.mass * self.min_thrust) + (hcl.cos(state[2]) / self.mass * self.max_thrust))) + spat_deriv[3]*(((-self.length_between_copters / self.moment_of_inertia * self.min_thrust) + (self.length_between_copters / self.moment_of_inertia * self.max_thrust)))
        SumLL[0] = spat_deriv[1]*(((hcl.cos(state[2]) / self.mass * self.min_thrust) + (hcl.cos(state[2]) / self.mass * self.min_thrust))) + spat_deriv[3]*(((-self.length_between_copters / self.moment_of_inertia * self.min_thrust) + (self.length_between_copters / self.moment_of_inertia * self.min_thrust)))
